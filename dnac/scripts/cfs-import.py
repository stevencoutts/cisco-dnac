...
def main():
    ...
    with dna.Dnac(HOST) as dnac:
        ...
        sgts = dnac.get("data/customer-facing-service/scalablegroup", ver="v2").response
        ...
        for host in set(r["Hostname"] for r in rows if r["Hostname"] != ""):
            ...
            for row in [r for r in rows if r["Hostname"] == host]:
                ...
                # Add interface
                    ...
                    data = dna.JsonObj(
                        {"interfaceId": interface.id, "role": "LAN", "segment": []}
                    )
                # Update fields
                if auth is not None:
            ...
            response = dnac.put(
                "data/customer-facing-service/DeviceInfo", ver="v2", data=[di]
            ).response
