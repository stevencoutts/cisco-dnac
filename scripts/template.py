...
def main():
    ...
    with dna.Dnac(HOST) as dnac:
        ...
        if template.templateParams:
            ...
            for tp in template.templateParams:
                ...
                if not v:
                    ...
                    v = ", ".join("%d-%d" % (r.minValue, r.maxValue) for r in tp.range)
                # Compose user prompt for parameter value input
                prompt = (
                    "%s %s [%s]" % (dtype, dname, v) if v else "%s %s" % (dtype, dname)
                )
        ...
        if not dna.find(template.deviceTypes, devices[idx].family, "productFamily"):
        ...
        dis = dnac.get(
            "data/customer-facing-service/DeviceInfo",
            ver="v2",
            params={"name": devices[idx].id},
        ).response
        if not dis:
        ...
        dis[0].customProvisions = [
            {"type": "USER_CLI_TEMPLATE_PROVISION", "payload": payload}
        ]
        # Commit changes
        logging.debug("data=" + json.dumps(dis))
        response = dnac.put(
            "data/customer-facing-service/DeviceInfo", ver="v2", data=dis
        ).response
